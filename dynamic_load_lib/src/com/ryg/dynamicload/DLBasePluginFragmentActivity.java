...
public class DLBasePluginFragmentActivity extends FragmentActivity implements DLPlugin {
    ...
    protected DLPluginInfo mPluginPackage;

    @Override
    public void attach(Activity proxyActivity, DLPluginInfo pluginPackage) {
    ...
    public void navigateTo(String pageClass){
		Log.w("plugin" , "host navigateTo "+pageClass);

    	try {
			Class<?> resClass = getPageClassByName(pageClass);
			DLBasePage resPage = (DLBasePage)resClass.newInstance();
			navigateTo(resPage);
		} catch (ClassNotFoundException e) {
			Log.w("plugin" , "page ClassNotFoundException");
			e.printStackTrace();
    ...
    protected void navigateTo(DLBasePage page){
		Log.w("plugin" , "parent navigateto"+page.getClass().getName());
    }
    
    protected Class<?> getPageClassByName(String pageClassName) throws ClassNotFoundException{
    	...
    	Class<?> dest = null;
    	if(loader!=null){
    		return loader.loadClass(pageClassName);
    	} else {
    		dest = Class.forName(pageClassName);
    	}
    	return dest;
    }
